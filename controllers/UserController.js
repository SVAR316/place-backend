...
module.exports = class UserController {
    ...
    async login(req, res) {
        ...
        let checkedParams = CommonService.checkParams([username, password])
        ...
        let answer = await UserService.login(username, password)
        CommonService.endTimer('login')
    ...
    async registration(req, res) {
        ...
        let checkedParams = CommonService.checkParams([username, password, email, role])
        if (checkedParams.length !== 0) {
        ...
        let answer = await UserService.registration(username, password, email, role)
    ...
    async updateUser(req, res) {
        ...
        let checkedParams = CommonService.checkParams([id, username, password, email, urlImage, role])
        if (checkedParams.length !== 0) {
        ...
        let answer = await UserService.updateUser(id, username, password, email, urlImage, role)
    ...
    async getUser(req, res) {
        ...
        let checkedParams = CommonService.checkParams([id])
        if (checkedParams.length !== 0) {
